...
export function SectionsDialog({
  ...
  const [selectedSections, setSelectedSections] = useState<Set<Section["id"]>>(
    new Set()
  )
  ...
  function selectAllSectionsOfType(type: Section["schedule_type_description"]) {
    setSelectedSections(
      new Set(
        sections
          .filter((section) => section.schedule_type_description === type)
    ...
    <DrawerDialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DrawerDialogContent className="p-0 max-w-5xl min-h-[95dvh] flex flex-col overflow-y-hidden">
        ...
        <div className="px-10 overflow-y-auto grow">
          <div className="text-sm h-4">
            {isLoading ? (
              ...
              <p className="text-muted-foreground text-center sm:text-left">
                We found {numSections}{" "}
                {numSections === 1 ? "section" : "sections"} for this course.
              </p>
            )}
          </div>
              ...
              {isLoading ? (
                ...
                <>
                  {numLectures > 0 && (
                    <Button
                      variant="secondary"
                      onClick={() => selectAllSectionsOfType("Lecture")}
                    >
                  ...
                  {numTutorials > 0 && (
                    <Button
                      variant="secondary"
                      onClick={() => selectAllSectionsOfType("Tutorial")}
                    >
                  ...
                  {numLaboratories > 0 && (
                    <Button
                      variant="secondary"
                      onClick={() => selectAllSectionsOfType("Laboratory")}
                    >
          ...
          <div className="mt-8">
            <p className="mb-4 font-bold">Or pick classes yourself...</p>
