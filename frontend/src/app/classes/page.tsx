  ...
  return (
    <PageLayout>
      <main className="pt-10">
        ...
        <div className="mt-10">
          {selectedTerm && <SearchResults query={query} term={selectedTerm} />}
        </div>
      </main>
    </PageLayout>
  )
    ...
    <div className="flex gap-8">
      {terms === undefined ? (
        ...
        <Alert className="bg-blue-50 border-blue-500">
          <AlertDescription>
            There are no classes available for registration at this time. Check
            back soon!
          </AlertDescription>
        </Alert>
      ) : (
        <RadioGroup
          defaultValue={terms[0].term}
        ...
        >
          {terms.map((term) => (
            <div className="flex items-center space-x-2" key={term.term}>
              ...
              <Label htmlFor={term.term}>{term.term_desc}</Label>
            </div>
          ))}
        </RadioGroup>
      )}
...
function SearchResults({ query, term }: { query: string; term: Term }) {
  ...
  const {
    ...
    isFetching,
    isError,
  } = useCourses(debouncedTerm?.term, debouncedQuery)
    ...
    <div>
      <p className="text-sm h-8 flex items-center text-muted-foreground">
        {isFetching ? (
          ...
          <span>
            An error occurred while searching for courses. Please try again
            later.
          </span>
        ) : (
          ...
          debouncedTerm && (
            <span>
              Found {courses.length} results for &ldquo;{debouncedQuery}&rdquo;
              in {debouncedTerm?.term_desc}
            </span>
          )
        )}
      </p>
